...
Private Const SH_TASAS As String = "Tasas Ref"
...
Private Const C_TR     As Long = 11    ' total return - se carga pero la macro no lo usa
Private Const C_PORT   As Long = 12
Private Const C_PESO   As Long = 13
...
Private Sub RV12_Controles(ByVal f As Long)
    ...
    With ws.Labels.Add(14, 92, 900, 16)
        .Caption = "Referencias fijas (no dependen de los filtros):  tasa BCRP / SOFR  ·  portafolio (promedio ponderado por peso)  ·  benchmark del fondo (su hoja, ultimo dato <= fecha de corte)."
...
Private Sub RV12_Calcular(ByVal f As Long)
    ...
    ' --- puntos de referencia: se toma el ULTIMO dato con fecha <= corte ---
    Dim rb As Long, rd As Double, ry As Double, rs As Double
    For mi = 1 To 2
        ' 1) tasa de referencia (BCRP / SOFR): hoja Tasas Ref
        rb = RV12_RefBase(f, mi, 1)
        wc.Cells(1, rb).Value = "REF_TASA_" & RV12_Mon(mi)
        If RV12_Tasa(mi, ry) Then
            wc.Cells(2, rb).Value = 0
            wc.Cells(2, rb + 1).Value = ry
            wc.Cells(2, rb + 2).Value = 0
        End If

        ' 2) portafolio actual (promedio ponderado de la data del fondo)
        ...
        ' 3) benchmark del fondo: su propia hoja, segun el mapa de Config
        rb = RV12_RefBase(f, mi, 3)
        wc.Cells(1, rb).Value = "REF_BMK_" & RV12_Mon(mi)
        If RV12_Bmk(f, mi, rd, ry, rs) Then
            wc.Cells(2, rb).Value = rd
            wc.Cells(2, rb + 1).Value = ry
            wc.Cells(2, rb + 2).Value = rs
        End If
    Next mi
End Sub

'======================================================================
'  REFERENCIAS (series de tiempo, una hoja por benchmark)
'  Hoja de benchmark: fila 2 encabezados, datos desde fila 3
'      A FECHA | B DUR | C YTW_% | D SPREAD_PBS
'  Hoja "Tasas Ref": fila 2 encabezados, datos desde fila 3
'      A FECHA | B PEN_% | C USD_%
'  En ambos casos se toma el ULTIMO registro con FECHA <= Config B3.
'======================================================================
Private Function RV12_Corte() As Double
    Dim cfg As Worksheet
    Set cfg = ThisWorkbook.Worksheets(SH_CFG)
    RV12_Corte = CDbl(Date)
    On Error Resume Next
    If IsDate(cfg.Range("B3").Value) Then RV12_Corte = CDbl(CDate(cfg.Range("B3").Value))
    On Error GoTo 0
End Function

' Mapa fondo/moneda -> nombre de hoja, leido de Config (encabezado fila 12, datos fila 13+)
Private Function RV12_HojaBmk(ByVal f As Long, ByVal mi As Long) As String
    Dim cfg As Worksheet, i As Long, sF As String, sM As String
    Set cfg = ThisWorkbook.Worksheets(SH_CFG)
    i = 13
    Do While Len(Trim$(CStr(cfg.Cells(i, 1).Value))) > 0 And i < 200
        sF = UCase$(Trim$(CStr(cfg.Cells(i, 1).Value)))
        sM = UCase$(Trim$(CStr(cfg.Cells(i, 2).Value)))
        If sF = "F" & f And sM = RV12_Mon(mi) Then
            RV12_HojaBmk = Trim$(CStr(cfg.Cells(i, 3).Value))
            Exit Function
        End If
        i = i + 1
    Loop
...
' Ultimo dato del benchmark con fecha <= corte
Private Function RV12_Bmk(ByVal f As Long, ByVal mi As Long, ByRef oDur As Double, _
                          ByRef oYtw As Double, ByRef oSpr As Double) As Boolean
    Dim wb2 As Worksheet, nm As String
    Dim n As Long, i As Long, fMax As Double, corte As Double, fd As Double

    RV12_Bmk = False
    oDur = 0: oYtw = 0: oSpr = 0

    nm = RV12_HojaBmk(f, mi)
    If Len(nm) = 0 Then Exit Function
    On Error Resume Next
    Set wb2 = ThisWorkbook.Worksheets(nm)
    On Error GoTo 0
    If wb2 Is Nothing Then Exit Function

    corte = RV12_Corte()
    n = wb2.Cells(wb2.Rows.Count, 1).End(xlUp).Row
    fMax = 0
    For i = 3 To n
        If IsDate(wb2.Cells(i, 1).Value) Then
            fd = CDbl(CDate(wb2.Cells(i, 1).Value))
            If fd <= corte And fd >= fMax Then
                fMax = fd
                oDur = RV12_Num(wb2.Cells(i, 2).Value)
                oYtw = RV12_Num(wb2.Cells(i, 3).Value)
                oSpr = RV12_Num(wb2.Cells(i, 4).Value)
                RV12_Bmk = True
            End If
        End If
    Next i
...
' Ultima tasa de referencia con fecha <= corte (columna B = PEN, C = USD)
Private Function RV12_Tasa(ByVal mi As Long, ByRef oYtw As Double) As Boolean
    Dim wt As Worksheet
    Dim n As Long, i As Long, fMax As Double, corte As Double, fd As Double
    ...
    oYtw = 0
    On Error Resume Next
    Set wt = ThisWorkbook.Worksheets(SH_TASAS)
    On Error GoTo 0
    If wt Is Nothing Then Exit Function

    corte = RV12_Corte()
    n = wt.Cells(wt.Rows.Count, 1).End(xlUp).Row
    fMax = 0
    For i = 3 To n
        If IsDate(wt.Cells(i, 1).Value) Then
            fd = CDbl(CDate(wt.Cells(i, 1).Value))
            If fd <= corte And fd >= fMax Then
                fMax = fd
                oYtw = RV12_Num(wt.Cells(i, 1 + mi).Value)
                RV12_Tasa = True
            End If
        End If
    Next i
...
Private Function RV12_Subtitulo() As String
    ...
    sFu = Trim$(CStr(cfg.Range("B7").Value))
    On Error GoTo 0
...
Private Function RV12_EtiqTasa(ByVal mi As Long) As String
    ...
    Set cfg = ThisWorkbook.Worksheets(SH_CFG)
    s = Trim$(CStr(cfg.Range(IIf(mi = 1, "B5", "B6")).Value))
