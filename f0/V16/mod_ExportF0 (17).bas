...
Sub ExportarDataF0()
    ...
    ' --- lista Reg S: excel "isins" (col B = ISIN, col D = marca Reg S) ---
    ' Los ISIN US solo entran al universo si estan en esta lista como Reg S.
    Dim dRegS As Object, wbI As Workbook, shI As Worksheet, yaAbierto As Boolean, rutaI As Variant
    Set dRegS = CreateObject("Scripting.Dictionary")
    yaAbierto = False
    For Each wbI In Application.Workbooks
        If InStr(1, LCase$(wbI.Name), "isin", vbTextCompare) > 0 Then
            yaAbierto = True
            Exit For
        End If
    Next wbI
    If Not yaAbierto Then
        Set wbI = Nothing
        rutaI = Application.GetOpenFilename("Excel,*.xls;*.xlsx;*.xlsm", , _
            "Elige el excel 'isins' (col B = ISIN, col D = Reg S). Cancelar = excluir todos los US")
        If VarType(rutaI) <> vbBoolean Then Set wbI = Workbooks.Open(CStr(rutaI), ReadOnly:=True, UpdateLinks:=0)
    End If
    If Not wbI Is Nothing Then
        Set shI = wbI.Worksheets(1)
        For r = 1 To shI.Cells(shI.Rows.Count, 2).End(xlUp).Row
            If Len(TxtSeg(shI.Cells(r, 2).Value)) > 0 And Len(TxtSeg(shI.Cells(r, 4).Value)) > 0 Then
                clave = LimpiarCod(TxtSeg(shI.Cells(r, 2).Value))
                If Len(clave) > 0 Then If Not dRegS.Exists(clave) Then dRegS.Add clave, 1
            End If
        Next r
        If Not yaAbierto Then wbI.Close SaveChanges:=False
    End If
    ...
    For r = 2 To ultV
        ...
        If dVto > 0 Then
            ...
            If entra And cRat > 0 Then
                ...
                End If
            End If
            ' ISIN: los XS entran siempre; los US solo si figuran como Reg S
            ' en el excel "isins" (col B ISIN, col D marca). Sin lista -> US fuera.
            If entra And cIsin > 0 Then
                If UCase$(Left$(TxtSeg(shV.Cells(r, cIsin).Value), 2)) = "US" Then
                    If Not dRegS.Exists(LimpiarCod(TxtSeg(shV.Cells(r, cIsin).Value))) Then
                        entra = False
                        nIsinUS = nIsinUS + 1
                        AnotarExcl shX, filaX, "ISIN US sin Reg S", shV, r, cTipo, cEmi, cNemo, cIsin, cCod, cMon, cVto
                    End If
                End If
            End If
            If entra Then
...
SigV:
    ...
    CfgFila cfg, k, "Criterio universo: ISIN XS incluidos; US solo si Reg S (excel isins)", True
    CfgFila cfg, k, "Lista Reg S cargada (ISINs con marca en col D)", dRegS.Count
    ...
    msg = msg & "Excluidos: vencidos " & nVencidos & " | sin fecha vcto legible " & nSinFecha & _
          " | otra moneda " & nNoSoles & " | sin rating " & nSinRat & " | rating A/A- " & nRatExcl & " | ISIN US sin RegS " & nIsinUS & vbCrLf
    msg = msg & "AUDITORIA: hoja 'Excluidos' lista cada fila del vector que no entro y su motivo." & vbCrLf
    msg = msg & "GRAFICOS: abre este archivo y corre GenerarScatterRV para crear la hoja RV." & vbCrLf
...
'============================================================
' SCATTER RV: se corre SOBRE un RV_F0 ya generado (con Data F0)
' Usa el libro activo si tiene la hoja; si no, pide el archivo.
' Re-ejecutable: borra la hoja RV anterior y la reconstruye.
'============================================================
Public Sub GenerarScatterRV()
    Dim wb As Workbook, ws As Worksheet, shT As Worksheet, cfg As Worksheet
    Dim ruta As Variant, uf As Long, bmk As String, r As Long

    On Error Resume Next
    Set ws = ActiveWorkbook.Worksheets("Data F0")
    On Error GoTo 0
    If ws Is Nothing Then
        ruta = Application.GetOpenFilename("Excel,*.xlsx;*.xlsm", , "Elige el RV_F0_AAAAMMDD.xlsx (el que tiene la hoja Data F0)")
        If ruta = False Then Exit Sub
        Set wb = Workbooks.Open(CStr(ruta))
        On Error Resume Next
        Set ws = wb.Worksheets("Data F0")
        On Error GoTo 0
        If ws Is Nothing Then
            MsgBox "Ese archivo no tiene la hoja 'Data F0'.", vbExclamation
            Exit Sub
        End If
    Else
        Set wb = ws.Parent
    ...
    On Error Resume Next
    Set shT = wb.Worksheets("Tasa Ref BCRP")
    Set cfg = wb.Worksheets("Config")
    On Error GoTo 0

    uf = ws.Cells(ws.Rows.Count, 1).End(xlUp).Row
    bmk = ""
    If Not cfg Is Nothing Then
        For r = 1 To cfg.Cells(cfg.Rows.Count, 1).End(xlUp).Row
            If InStr(CStr(cfg.Cells(r, 1).Value), "Benchmark F0") > 0 Then
                bmk = Trim$(CStr(cfg.Cells(r, 2).Value))
                Exit For
            End If
        Next r
    End If
    ...
    Application.ScreenUpdating = False
    ConstruirRV wb, ws, shT, uf, bmk
    Application.ScreenUpdating = True

    On Error Resume Next
    wb.Save
    On Error GoTo 0
    MsgBox "Hoja 'RV' lista: 4 scatters con sus filtros, sobre " & wb.Name & vbCrLf & _
           "(archivo guardado; puedes recorrer esta macro cuando quieras y se reconstruye)", vbInformation
End Sub
...
Private Sub ConstruirRV(wbN As Workbook, ws As Worksheet, shT As Worksheet, _
    ...
    ' re-ejecutable: borrar la hoja RV anterior y limpiar los auxiliares viejos
    Dim hojaPrev As Worksheet
    On Error Resume Next
    Set hojaPrev = wbN.Worksheets("RV")
    On Error GoTo 0
    If Not hojaPrev Is Nothing Then
        Application.DisplayAlerts = False
        hojaPrev.Delete
        Application.DisplayAlerts = True
    End If
    ws.Range(ws.Columns(B1), ws.Columns(CENP)).EntireColumn.Hidden = False
    ws.Range(ws.Columns(B1), ws.Columns(CENP)).ClearContents
