...
Private Const DIAS_MAX_CORP As Long = 366       ' corporativos y depositos: hasta 1 ano al vencimiento
...
Sub ExportarDataF0()
    ...
    For r = 2 To ultV
        ...
        If IsDate(vto) Then
            ...
            entra = False
            If dias > 0 Then
                If cat = "SOBERANO" And SOB_TODO_PLAZO Then
                    entra = True
            ...
            If entra And cMon > 0 Then
                If InStr(1, UCase$(CStr(shV.Cells(r, cMon).Value)), MONEDA_UNIV, vbTextCompare) = 0 Then entra = False
            End If
            If entra Then
    ...
    CfgFila cfg, k, "Criterio universo: moneda", MONEDA_UNIV
    CfgFila cfg, k, "Criterio universo: dias max al vcto (corp y depositos)", DIAS_MAX_CORP
    ...
    If Err.Number <> 0 Then
        MsgBox "No pude guardar en:" & vbCrLf & destino & vbCrLf & vbCrLf & _
               "Error " & Err.Number & ": " & Err.Description & vbCrLf & vbCrLf & _
               "Causas tipicas: el archivo ya esta abierto, la carpeta no existe, " & _
               "es una ruta de OneDrive/SharePoint, o no hay permiso de escritura.", vbExclamation
        Err.Clear
        On Error GoTo 0
        alt = Application.GetSaveAsFilename("RV_F0_" & fFMS & ".xlsx", "Excel,*.xlsx", 1, "No pude guardar en la carpeta automatica. Elige donde guardar")
        If alt <> False Then
            On Error Resume Next
            wbN.SaveAs Filename:=CStr(alt), FileFormat:=xlOpenXMLWorkbook
            If Err.Number <> 0 Then
                Err.Clear
                destino = "(no se pudo guardar; el libro quedo abierto sin grabar)"
                MsgBox "Tampoco pude guardar en la ruta elegida.", vbExclamation
            Else
                destino = CStr(alt)
            End If
            On Error GoTo 0
