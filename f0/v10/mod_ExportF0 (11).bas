...
Sub ExportarDataF0()
    ...
    For r = 4 To ultD
        If Len(Trim$(CStr(shD.Cells(r, 2).Value))) > 0 Then
            ...
            ws.Cells(fila, 24).Value = ACNorm(CStr(shD.Cells(r, 1).Value))
    ...
    For r = 2 To ultV
        ...
        hayInstr = (Len(Trim$(CStr(shV.Cells(r, cCod).Value))) > 0)
        If Not hayInstr And cNemo > 0 Then hayInstr = (Len(Trim$(CStr(shV.Cells(r, cNemo).Value))) > 0)
        If Not hayInstr And cIsin > 0 Then hayInstr = (Len(Trim$(CStr(shV.Cells(r, cIsin).Value))) > 0)
        If Not hayInstr And cEmi > 0 Then hayInstr = (Len(Trim$(CStr(shV.Cells(r, cEmi).Value))) > 0)
        ...
        If dVto > 0 Then
            ...
            If entra And cMon > 0 Then
                If Not EsSoles(CStr(shV.Cells(r, cMon).Value)) Then
                    entra = False
                    nNoSoles = nNoSoles + 1
            ...
            If entra Then
                ...
                Else
                    ...
                    ws.Cells(fila, 24).Value = ACNorm(CStr(shV.Cells(r, cTipo).Value))
                    ws.Cells(fila, 21).Value = fFMS
                    ws.Cells(fila, 22).Value = fVec
                    nUniv = nUniv + 1
                    fila = fila + 1
...
Private Sub AnotarExcl(shX As Worksheet, ByRef filaX As Long, motivo As String, _
    ...
    If cTipo > 0 Then shX.Cells(filaX, 2).Value = shV.Cells(r, cTipo).Value
    If cEmi > 0 Then shX.Cells(filaX, 3).Value = shV.Cells(r, cEmi).Value
    If cNemo > 0 Then shX.Cells(filaX, 4).Value = shV.Cells(r, cNemo).Value
    If cIsin > 0 Then shX.Cells(filaX, 5).Value = shV.Cells(r, cIsin).Value
    shX.Cells(filaX, 6).Value = shV.Cells(r, cCod).Value
    If cMon > 0 Then shX.Cells(filaX, 7).Value = shV.Cells(r, cMon).Value
    shX.Cells(filaX, 8).Value = shV.Cells(r, cVto).Value
    filaX = filaX + 1
End Sub
